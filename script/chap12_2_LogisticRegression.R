...
# 값의범위 : 0 ~ 1 확률값(cut off=0.5) -> 이항분류 적합
...
# family = 'binomial' : 정규분포인 경우 gaussian, 이항분포인 경우 binomial, 
# 포아송분포인 경우 poisson, 역정규분포인 경우 inverse.gaussian, 감마분포인 경우 gamma, 
# 그리고 응답분포가 확실하지 않은 때를 위한 유사가능도 모형인 경우 quasi를 사용할 수 있다.
weater_model 
...
pred 
range(pred, na.rm = T) # 0.000497783 ~ 0.989092637
summary(pred)
str(pred)
...
# model 평가 : 분류정확도 
acc <- (79+10) / sum(t)
cat('accuracy =', acc) # accuracy = 0.8165138

# 특이도 : 정답이 no일 때 no를 예측한 확률
acc_no <- 79 / 91 # 0.8681319
# 재현률 : 정답이 yes일 때 yes를 예측한 확률
recall <- 10 / 18 # 0.5555556
# 정확률 : model(yes) -> yes
precision <- 10 / 22 # 0.4545455
...
# 이항분류 vs 다항분류
# sigmoid : 이항분류 (0~1사이 확률값, cut-off = 0.5)
# softmax : 다항분류 (0~1사이 확률값, 확률합 = 1)
...
# type = "probs" : 0~1확률(합=1)
y_pred <- predict(model, test_iris, type = "class")
# type = "class" : 
range(y_pred) # 2.034338e-36(0) ~ 1.000000e+00(1)
