...
# age 변수 이상치 처리
summary(dataset$age) # NA's 31 발견
dataset2 <- subset(dataset2, age >= 20 & age <= 69)
plot(dataset2$age)

# 3. 코딩 변경 : 최초 코딩 내용을 목적에 맞게 변경
# - 데이터 가독성, 척도 변경
...
dataset2$resident2[dataset2$resident==2] <- "2.이천시"
dataset2$resident2[dataset2$resident==3] <- "3.천안시"
dataset2$resident2[dataset2$resident==4] <- "4.군산시"
dataset2$resident2[dataset2$resident==5] <- "5.김해시"

table(dataset2$resident2)
